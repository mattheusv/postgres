...
PLy_exec_function(FunctionCallInfo fcinfo, PLyProcedureCache *pcache)
{
	...
	PG_CATCH();
	{
		...
		/*
		 * If the error was thrown within a SRF, clean up its state here.
		 * This is the only cleanup hook that runs for an error thrown during
		 * the function's own execution: ShutdownPLyFunction is not called on
