					...
					const TupleTableSlotOps *ops, int natts)
{
	...
	/* Create the signature and function */
	{
		...
		param_types[0] = l_ptr(types->StructTupleTableSlot);
	...
	llvm_copy_attributes(types->AttributeTemplate, v_deform_fn);
	...
	v_offp = LLVMBuildAlloca(b, types->TypeSizeT, "v_offp");

	v_slot = LLVMGetParam(v_deform_fn, 0);

	v_tts_values =
		l_load_struct_gep(b, types->StructTupleTableSlot, v_slot, FIELDNO_TUPLETABLESLOT_VALUES,
						  "tts_values");
	v_tts_nulls =
		l_load_struct_gep(b, types->StructTupleTableSlot, v_slot, FIELDNO_TUPLETABLESLOT_ISNULL,
						  "tts_ISNULL");
	v_flagsp = l_struct_gep(b, types->StructTupleTableSlot, v_slot, FIELDNO_TUPLETABLESLOT_FLAGS, "");
	v_nvalidp = l_struct_gep(b, types->StructTupleTableSlot, v_slot, FIELDNO_TUPLETABLESLOT_NVALID, "");
	...
	if (ops == &TTSOpsHeapTuple || ops == &TTSOpsBufferHeapTuple)
	{
		...
		v_heapslot =
			LLVMBuildBitCast(b,
							 v_slot,
							 l_ptr(types->StructHeapTupleTableSlot),
							 "heapslot");
		v_slotoffp = l_struct_gep(b, types->StructHeapTupleTableSlot, v_heapslot, FIELDNO_HEAPTUPLETABLESLOT_OFF, "");
		v_tupleheaderp =
			l_load_struct_gep(b, types->StructHeapTupleTableSlot, v_heapslot, FIELDNO_HEAPTUPLETABLESLOT_TUPLE,
							  "tupleheader");
	}
	else if (ops == &TTSOpsMinimalTuple)
	{
		...
		v_minimalslot =
			LLVMBuildBitCast(b,
							 v_slot,
							 l_ptr(types->StructMinimalTupleTableSlot),
							 "minimalslot");
		v_slotoffp = l_struct_gep(b,
								  types->StructMinimalTupleTableSlot,
								  v_minimalslot,
	...
	v_bits =
		LLVMBuildBitCast(b,
						 l_struct_gep(b,
									  types->StructHeapTupleHeaderData,
									  v_tuplep,
	 ...
	 */
	{
		...
		v_off_start = LLVMBuildZExt(b, v_off_start, types->TypeSizeT, "");
	...
	for (attnum = 0; attnum < natts; attnum++)
	{
		...
		if (attnum == 0)
		{
			LLVMBuildStore(b, l_sizet_const(types, 0), v_offp);
		}
		...
		if (att->attnullability != ATTNULLABLE_VALID)
		{
			...
			LLVMBuildStore(b,
						   ...
						   l_gep(b, types->TypeDatum, v_tts_values, &l_attno, 1, ""));
			...
			(known_alignment < 0 || known_alignment != TYPEALIGN(alignto, known_alignment)))
		{
			...
			if (att->attlen == -1)
			{
				...
				v_off = l_load(b, types->TypeSizeT, v_offp, "");
			...
			/* translation of alignment code (cf TYPEALIGN()) */
			{
				...
				LLVMValueRef v_off = l_load(b, types->TypeSizeT, v_offp, "");

				/* ((ALIGNVAL) - 1) */
				LLVMValueRef v_alignval = l_sizet_const(types, alignto - 1);

				/* ((uintptr_t) (LEN) + ((ALIGNVAL) - 1)) */
				LLVMValueRef v_lh = LLVMBuildAdd(b, v_off, v_alignval, "");

				/* ~((uintptr_t) ((ALIGNVAL) - 1)) */
				LLVMValueRef v_rh = l_sizet_const(types, ~(alignto - 1));
		...
		{
			Assert(known_alignment >= 0);
			LLVMBuildStore(b, l_sizet_const(types, known_alignment), v_offp);
		}
		...
		v_resultp = l_gep(b, types->TypeDatum, v_tts_values, &l_attno, 1, "");

		/* store null-byte (false) */
		LLVMBuildStore(b, l_int8_const(lc, 0),
					   l_gep(b, types->TypeStorageBool, v_tts_nulls, &l_attno, 1, ""));
		...
		if (att->attbyval)
		{
			...
			v_tmp_loaddata = LLVMBuildSExt(b, v_tmp_loaddata, types->TypeDatum, "");
		...
		if (att->attlen > 0)
		{
			v_incby = l_sizet_const(types, att->attlen);
		}
		...
		else if (att->attlen == -2)
		{
			...
			/* add 1 for NUL byte */
			v_incby = LLVMBuildAdd(b, v_incby, l_sizet_const(types, 1), "");
		}
		else
		{
			Assert(false);
